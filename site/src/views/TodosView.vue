...
const filteredTodos = computed(() => {
  if (filter.value === 'active')
    return todos.value.filter((t) => !t.isComplete);
...
function toApiDate(dateStr: string) {
  // dateStr is yyyy-mm-dd from <input type="date">. Treat it as a pure calendar
  // date pinned to UTC midnight — never run it through new Date(), which would
  // reinterpret it in the local timezone and shift the day.
...
async function submitCreate() {
  ...
  try {
    const { data } = await api.post<Todo>('/api/todos', {
      ...
      dueDate: toApiDate(createForm.dueDate),
    });
...
async function submitEdit(todo: Todo) {
  ...
  try {
    const { data } = await api.put<Todo>(`/api/todos/${todo.id}`, {
      ...
      dueDate: toApiDate(editForm.dueDate),
...
async function toggleComplete(todo: Todo) {
  ...
  try {
    ...
    pageError.value = apiError(
      e,
      'Failed to update task. Your change was not saved.',
    );
  }
}
...
<template>
  <div class="max-w-2xl mx-auto px-4 py-10">
    ...
    <div
      v-if="pageError"
      class="text-sm text-error bg-error-light px-4 py-3 rounded mb-5 flex justify-between"
    >
      <span>{{ pageError }}</span>
      <button @click="pageError = ''" class="ml-4 opacity-60 hover:opacity-100">
        ✕
      </button>
    </div>
    ...
    <div
      v-if="showCreateForm"
      class="bg-card rounded-lg shadow-card p-6 mb-6 border border-taupe-light"
    >
      <h2 class="font-serif text-lg text-charcoal mb-4">New Task</h2>
      <form @submit.prevent="submitCreate" class="space-y-4">
        <div>
          <label
            class="block text-xs font-medium text-charcoal mb-1 uppercase tracking-wide"
          >
        ...
        <div
          v-if="createError"
          class="text-sm text-error bg-error-light px-3 py-2 rounded"
        >
          {{ createError }}
        </div>
    ...
    <div class="flex gap-0 mb-5 border-b border-taupe-light">
      ...
      >
        ...
        <span
          class="ml-1 text-xs"
          :class="filter === tab ? 'text-taupe' : 'text-taupe-light'"
        >
          {{ counts[tab] }}
        </span>
      </button>
    ...
    <!-- Loading -->
    <div v-if="loading" class="text-center py-20 text-taupe text-sm">
      Loading…
    </div>
    ...
    <div v-else-if="filteredTodos.length === 0" class="text-center py-20">
      <p class="text-taupe text-sm">
        <template v-if="filter === 'completed'"
          >No completed tasks yet.</template
        >
        <template v-else-if="filter === 'active'"
          >No active tasks — you're all caught up!</template
        >
    ...
    <ul v-else class="space-y-3">
      ...
      >
        ...
        <div v-if="editingId !== todo.id" class="flex items-start gap-3 p-4">
          ...
          <div class="flex-1 min-w-0">
            <p
              ...
              :class="
                todo.isComplete ? 'line-through text-taupe' : 'text-charcoal'
              "
            >
              {{ todo.title }}
            </p>
            <p
              ...
              class="text-xs text-taupe mt-1 leading-relaxed"
            >
              {{ todo.description }}
            </p>
            <p
              ...
              "
            >
              <span v-if="isOverdue(todo.dueDate, todo.isComplete)"
                >Overdue ·
              </span>
        ...
        <form
          v-else
          @submit.prevent="submitEdit(todo)"
          class="p-4 space-y-3 bg-background/50"
        >
          <input
          ...
          <div
            v-if="editError"
            class="text-xs text-error bg-error-light px-3 py-2 rounded"
          >
            {{ editError }}
          </div>
